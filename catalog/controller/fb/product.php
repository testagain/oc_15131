<?php  
class ControllerFbProduct extends Controller {
	private $error = array(); 
	...
	public function index(){
		...
		$this->data['heading_title'] = (strlen($product_info['name'])>100)?substr($product_info['name'], 0, 100).'...':$product_info['name'];
			...
			$this->data['redirect'] = HTTP_SERVER . 'index.php?route=product/product&product_id=' . $this->request->get['product_id'];
			...
			foreach ($results as $result) {
				...
				if ($options) {
					$add = $this->model_tool_seo_url->rewrite(HTTP_SERVER . 'index.php?route=fb/product&product_id=' . $result['product_id']);
				} else {
			...
			$this->template = 'fb/template/product/product.tpl';
